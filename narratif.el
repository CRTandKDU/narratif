...
;; This function generated by Duck.AI
(defun narratif--display-buffer-full-size (buffer alist)
  "Display BUFFER in a full-size window."
  (let ((window (display-buffer-pop-up-window buffer alist)))
    (when window
      (delete-other-windows window)  ; Make the window full size
      window)))

(defun narratif (org-source-file)
  (interactive "fSelect story Org file: ")
  (add-to-list 'display-buffer-alist
             '("\\*NARRATIF\\*" narratif--display-buffer-full-size))
  (with-current-buffer (get-buffer-create "*STORYFILE*")
    (erase-buffer)
    (insert-file-contents org-source-file)
    )
  (narratif-init (get-buffer "*STORYFILE*")))
